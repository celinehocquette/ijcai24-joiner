...
%prop(abc_b,(does,c_player)).
%:- prop(abc_b,(P,Q)), body_literal(Rule,P,_,(A,B,C)), body_literal(Rule,Q,_,(B,)), valid_var(Rule,B).

valid_var(Rule,Var):-
     obeys_datalog_check(Rule,Var).

 %% if non_datalog and allow_singletons are true, all vars are valid
 obeys_datalog_check(Rule,Var):-
     non_datalog,
     allow_singletons,
     clause_var(Rule,Var).

 %% if non_datalog is false and allow_singletons is true, a body only variable is valid
 obeys_datalog_check(Rule,Var):-
     not non_datalog,
     allow_singletons,
     clause_var(Rule,Var),
