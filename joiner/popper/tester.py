...
from . util import format_rule, order_rule, order_prog, prog_is_recursive, format_prog, format_literal, rule_is_recursive, flatten
...
class Tester():
    ...
    def test_prog_neg(self, prog):
        if len(prog) == 1:
            return self.test_single_rule_neg(prog)
        try:
            with self.using(prog):
    ...
    def reduce_inconsistent(self, program):
        ...
        for i in range(len(program)):
            ...
            uncombined = [k for k in subprog if not isinstance(k, Combination)]
            combined = [k.unfolded for k in subprog if isinstance(k, Combination)]
            subprog_folded = flatten(uncombined+combined)
            if not prog_is_recursive(subprog_folded):
                continue
            if self.is_inconsistent(subprog_folded):
                return self.reduce_inconsistent(subprog)
        return program

    def is_sat(self, prog, min_exs=0):
        if len(prog) == 1:
            rule = list(prog)[0]
            head, _ = rule
            head, ordered_body = order_rule(rule, self.settings)
            if min_exs > 0:
                ...
                x = f'succeeds_k_times({new_head},({x}),{min_exs+1}).'
