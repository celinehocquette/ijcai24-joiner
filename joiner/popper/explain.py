...
class Explainer:
    ...
    def explain_totally_incomplete(self, prog, directions, min_exs=0):
        return self.explain_totally_incomplete_aux(prog, directions, 0, defaultdict(list), defaultdict(list), min_exs=min_exs)

    def explain_totally_incomplete_aux(self, prog, directions, depth, sat=defaultdict(list), unsat=defaultdict(list), min_exs=0):
        ...
        for subprog in find_subprogs(prog, has_recursion):
            ...
            if seen_more_general_unsat(raw_prog, unsat[min_exs]):
                continue

            if seen_more_specific_sat(raw_prog, sat[min_exs]):
                continue
            ...
            if headless:
                ...
                if self.tester.is_body_sat(order_body(body)):
                    sat[min_exs].append(raw_prog)
                    continue
            else:
                if self.tester.is_sat(test_prog, min_exs=min_exs):
                    sat[min_exs].append(raw_prog)
                    continue

            unsat[min_exs].append(raw_prog)
            xs = list(self.explain_totally_incomplete_aux(subprog, directions, depth+1, sat, unsat, min_exs=min_exs))
            if len(xs):
                yield from xs
            else:
...
def order_body(body):
    ...
    while body_literals:
        ...
        if selected_literal == None:
            message = f'{selected_literal} in clause could not be grounded'
