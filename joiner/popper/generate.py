...
NONSPLITTABLE = """
%%% this constraint is the main difference with normal Popper: we disallow splittable rules unless the program is
%%% recursive or has pi
...
path(C,Var1,Var2) :- path1(C,Var1,Var3), path(C,Var3,Var2).

%% we also disallow rules with multiple body literals when a body literal only has head variables
:- not pi_or_rec, body_literal(C,P1,_,Vars1), body_literal(C,P2,_,Vars2), (P1,Vars1) != (P2,Vars2), head_only_vars(C,Vars1).
head_only_vars(C,Vars) :- body_literal(C,_,_,Vars), clause(C), not has_body_var(C,Vars). 
has_body_var(C,Vars) :- var_member(Var,Vars), body_not_head(C,Var).
body_not_head(C,V) :- body_var(C,V), not head_var(C,V).

%% DATALOG
%% programs with more than one rule should be datalog
:-  not non_datalog,
    clause(1),
    head_var(C,Var),
    not body_var(C,Var).
...
class Generator:
    ...
    def __init__(self, settings, grounder, bkcons=[]):
        ...
        if self.settings.disallowsplittable:
            alan += NONSPLITTABLE
        else:
            alan += DATALOG_SPLITTABLE
