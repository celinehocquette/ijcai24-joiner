...
next_score(A,B,C):- c95(C),c95(C),my_true_score(A,B,D),c70(D),end_game(E),does(A,F,E),opponent(B,F).
next_score(A,B,C):- c90(C),c90(C),my_true_claim_made_by(A,B),c65(D),my_true_score(A,B,D),opponent(B,F),does(A,F,E),end_game(E).
% accuracy: 75.0
% learning time: 600
% program size: 40
