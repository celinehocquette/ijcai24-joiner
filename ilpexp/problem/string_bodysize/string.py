...
def gen_pos(substrings):
    s = gen_string(min_length=len(substrings)+1)
...
class StringBodySizeProblem(Problem):
    ...
    def __init__(self, n_substrings, len_substrings, num_examples=DEFAULT_NUM_EXAMPLES):
        super().__init__(f"stringbodysize_{len_substrings}", parameter=f"{n_substrings}")
        self.n_substrings = n_substrings
        self.len_substrings = len_substrings
        self.substrings = [gen_substring(min_length=len_substrings, max_length=len_substrings) for _ in range(n_substrings)]
        self.gen_pos = partial(gen_pos, self.substrings)
        self.gen_neg = partial(gen_neg, self.substrings)
        self.sub_dir = f"stringbodysize_{len_substrings}"
